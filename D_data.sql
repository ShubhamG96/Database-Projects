use D_data
-- Creating Table structure for  Categories table --
--
...
-- Creating Table structure for Customers table --
...
-- Creating Table structure for employees table --
...
-- Creating Table structure for orders table --
CREATE TABLE orders (
...
-- Creating Table structure for order_details table --
...
-- Creating Table structure for products table 
...
-- Creating Table structure for shippers table  
CREATE TABLE shippers (
  ShipperID int NOT NULL,
  ShipperName varchar(255) DEFAULT NULL,
  Phone text DEFAULT NULL
)
-- -- Inserting data in shippers table  --
INSERT INTO shippers (ShipperID, ShipperName, Phone) VALUES
(1, 'Speedy Express', '(503) 555-9831'),
(2, 'United Package', '(503) 555-3199'),
(3, 'Federal Shipping', '(503) 555-9931');

-- Creating Table structure for suppliers table 
...
--Assigning Index(Primary Key) to Exsiting Table--
...
--16.Select name,price of the cheapest product (only name) without using LIMIT and OFFSET
...
--3. Select customers who did not placed any order yet
SELECT customers.CustomerID, CustomerName, OrderID FROM customers 
...
--5. Select products that no one bought
select products.ProductID,products.ProductName,order_details.OrderID from products
left join order_details on products.ProductID=order_details.ProductID 
where order_details.OrderID is null

--6. Select customer together with the products that he bought
select customers.CustomerID,customers.CustomerName,products.ProductID ,products.ProductName from customers 
...
--7. Select product names together with the name of corresponding category
select products.ProductID,products.ProductName,categories.Category_Name from products 
join categories on products.CategoryID=categories.Category_ID

--8. Select orders together with the name of the shipping company
select orders.OrderID,orders.CustomerID,shippers.ShipperID,shippers.ShipperName from orders
join shippers on orders.ShipperID=shippers.ShipperID

--9. Select customers with id greater than 50 together with each order they made
select customers.CustomerID,customers.CustomerName,orders.OrderID from customers 
join orders on customers.CustomerID=orders.CustomerID 
where orders.OrderID > 50 
order by OrderID asc;

--10. Select employees together with orders with order id greater than 10400
select employees.EmployeeID,employees.FirstName,employees.LastName,orders.OrderID from employees 
join orders on orders.EmployeeID=employees.EmployeeID 
where OrderID >10400 
order by OrderID asc;

--11. Select the most expensive product
SELECT TOP 1 ProductID, ProductName, Price FROM products 
ORDER BY Price DESC;

--12. Select the second most expensive product
SELECT TOP 2 ProductID, ProductName, Price FROM products 
ORDER BY Price DESC;
...
--14. Select 5 most expensive products
select Top 5 products.ProductID,products.ProductName,products.Price from products 
order by Price DESC


--15. Select 5 most expensive products without the most expensive (in final 4 products)
select products.ProductID,products.ProductName,products.Price from products 
order by Price DESC
...
--19.Select customer name together with the number of orders made by the corresponding customer sort the result by number of orders in decreasing order
select customers.CustomerName,COUNT(OrderID) AS "Total order",orders.OrderID from customers 
join orders on customers.CustomerID=orders.CustomerID 
...
--20. Add up the price of all products
select sum(price) as "Total price of all product" from products

--21. Select orderID together with the total price of  that Order, order the result by total price of order in increasing order
select order_details.OrderID,products.Price,
sum( order_details.Quantity * products.price) as "Total Price of the orders" 
from order_details 
...
sum( order_details.Quantity * products.price)as "Spend most money" 
from customers 
join products on customers.CustomerID = products.CategoryID 
join order_details on order_details.ProductID = products.ProductID
group by CustomerID,CustomerName

--23. Select customer who spend the most money and lives in Canada
select CustomerName,sum( order_details.Quantity * products.Price) as "Spend most money",customers.Country
from orders 
...
order by [Spend most money] DESC

--24.Select customer who spend the second most money
select CustomerName,sum( order_details.Quantity * products.Price) as "Spend most money"
from orders 
join customers on customers.CustomerID = orders.CustomerID 
join order_details on order_details.OrderID=orders.OrderID
join products on products.ProductID=order_details.ProductID
...
--25.Select shipper together with the total price of proceed orders
select shippers.ShipperName,sum( order_details.Quantity * products.Price) as "total price of processed orders" 
from shippers 
join orders on orders.ShipperID=shippers.ShipperID 
join order_details on order_details.OrderID = orders.OrderID
join products on products.ProductID=order_details.ProductID
group by ShipperName
order by [total price of processed orders] desc
